...
所有的装备都有一定的属性，将其与三种战斗风格之一联系起来：[近战](#), [远程](#), 或 [魔法](#)。通常来说，更难获得的装备 *通常* 更好。接近游戏末期，选择变得更加情境化。
...
[https://zxv975.github.io/CanIIdle/ 我能闲置] 是由Silber制作的网站。您可以输入在战斗页面找到的一些属性，然后查看您是否可以闲置哪些敌人。这是游戏中非常有用的工具。自v1.1以来，该网站不再更新任何超出基础游戏的内容，但对于尚未购买任何扩展内容的玩家来说，它仍然完全准确。对于已完成{{ZoneIcon|先兆黑暗事件}}并购买了{{TotH|使者的王座扩展}}的玩家，它只会在精英/大师级{{技能|屠杀}}任务方面不准确。
...
最后，使用[[战斗模拟器]]和[https://zxv975.github.io/CanIIdle/ 我能闲置]应该是您的首选，而不是印刷在这里的任何内容。本指南不对不及时的死亡负责。

===近战===
====升级====
大多数玩家从{{技能|攻击|近战}}开始战斗，因为它不需要{{技能|弓箭制作}}近战武器的弹药，也不需要{{技能|符文锻造}}魔法武器的符文。

对于近战，使用{{技能|锻造}}来穿戴和挥舞最高级别的装备。
...
由于游戏中{{技能|Attack|Melee}}敌人极为常见，魔法是一种耗时但有回报的战斗风格。对付游戏中最常见的敌人造成更多伤害并减少受到的伤害非常棒。
...
由于{{MonsterIcon|Bane}}使用来自你与他战斗的杀手区域的修改器，所以你应该总是在{{ZoneIcon|Perilous Peaks}}中与他战斗。你的祈祷将是{{PrayerIcon|Battleheart}}和{{PrayerIcon|Protect from Melee|notext=true}}{{PrayerIcon|Protect from Ranged|notext=true}}{{PrayerIcon|Protect from Magic|Protect from Style}}，这使区域效果（-% 闪避）实际上不会影响{{MonsterIcon|Bane|Bane}}击中玩家的几率。

与{{MonsterIcon|Bane}}的最后一场战斗是一个特殊版本，{{MonsterIcon|Bane, Instrument of Fear}}，具有额外的能力。

每次{{MonsterIcon|Bane}}被击败后，以下奖励将直接给予玩家，按照以下顺序设置。所有这些都是基本游戏中的最佳选择，仍然在任何扩展内容中有用且可行：
...
* {{ItemIcon|屠杀技能披风}} 或 {{ItemIcon|最大技能披风}}。
* 在被动槽中放置{{ItemIcon|炽热灯笼}}。
* 在被动槽中放置{{ItemIcon|攀登靴}}。

Q: 这个地下城需要多长时间？
答：大约需要2小时，具体取决于您的装备和准备情况。地下城的每个阶段（共5个阶段）应该需要大约20分钟的战斗时间和5-10分钟的装备更换、能力阅读和休息时间。

Q: 我需要完成这个活动多少次？
答：只需要杀死{{MonsterIcon|恐惧之仪的祸害}}一次。请注意，在达到最终首领之前，必须杀死{{MonsterIcon|祸害}}四次。
...
{{TotH|使者的王座扩展}}中的战斗遵循与基础游戏类似的进程，分为三个部分：早期游戏、中期游戏和使者的王座的晚期游戏。

本指南的整个部分都可能会发生变化，并将随着时间的推移进行更新，但目前遵循一些基本准则是一个很好的入门方式：使者的王座扩展中的战斗主要围绕着使用[屠杀](#)区域来准备完成地下城展开。升级可以在前两个屠杀区域——{{ZoneIcon|丛林迷宫}}和{{ZoneIcon|熔岩湖}}中进行，需要注意的是，[[致命毒药]]只是[[毒药]]的更高伤害版本，它在持续时间内造成的伤害是玩家生命值的25%，而不是10%。

一个简单的{{TotH}}战斗完成顺序：
# {{ZoneIcon|丛林迷宫}}，以获取{{UpgradeIcon|迷宫解决方案}}
# {{ZoneIcon|远古圣所}} - 参见[[远古圣所/指南]]
# {{ZoneIcon|熔岩湖}}和{{ZoneIcon|被遗忘的冻土}}
# {{ZoneIcon|地下熔岩湖}}和{{ZoneIcon|电光区}}地下城
# *可选*：{{ZoneIcon|蜘蛛女皇的巢穴}} - 此阶段必须手动清除。
# {{ZoneIcon|雾湖}}，获取强大的中期工具
# {{ZoneIcon|午夜山谷}}，获取强大的中期武器
# {{ZoneIcon|诅咒森林}}和{{ZoneIcon|死灵法师宫殿}}，获取强大的晚期装备
# {{ZoneIcon|金云山脉}}，仅在获得{{UpgradeIcon|金色碎片}}之后
# {{ZoneIcon|千年之门}}，获得具有最高伤害减免的BIS末期护甲
...
* {{ItemIcon|卓越的最大技能披风}}是对已经完全升级了所有技能的玩家的奖励，是99级{{ItemIcon|最大技能披风}}的直接升级。

==属性与增益==
===星象学===
在玩家的冒险过程中，可以安全地假设他们已经解锁了[星象学](#)的绝大部分奖励，如果玩家刚开始进入扩展并且还没有完成星象学，那么可以尽快获取{{ConstellationIcon|阿斯塔}}提供的一些重大好处，以及{{ConstellationIcon|罗萨妮雅}}提供的一些好处。

当然，清除{{ZoneIcon|诅咒森林}}以获取{{UpgradeIcon|星辰之印}}是有帮助的，但这需要深入到扩展战斗中，可能是不必要的。同样，在击败{{MonsterIcon|恐惧之仪|Bane, Instrument of Fear}}后，将解锁{{MonsterIcon|恶魔}}的[城镇](#)崇拜，提供强大的（{{Icon|傍晚|type=township}}）季节，将[星象学](#)的间隔缩短了50%，持续3天。
...
| This grants the effect of {{ItemIcon|Diamond Luck Potion IV}} for an generally inconsequential penalty of [Hitpoints](#).
|-
...
当{{AgilityIcon|瀑布十字路口}}生效时，{{ItemIcon|钻石幸运药水 IV}}不再是提供的伤害量所必需的，这为玩家选择使用哪种药水开启了许多新的选项。

* {{ItemIcon|近战力量药水 IV|notext=true}}{{ItemIcon|远程力量药水 IV|notext=true}}{{ItemIcon|魔法伤害药水 IV|notext=true}} 最大打击药水可以成为增加伤害的可靠来源。
* {{ItemIcon|减伤药水 IV}} 对于满足减伤需求非常有用，尤其是对于后期扩展的怪物和地牢。
* {{ItemIcon|杀戮赏金药水 IV}} 对于农场{{SC|SC}}非常有用的强大增益效果。
* {{ItemIcon|区域控制药水 IV}} 对于抵消有害的杀戮区域效果（如{{ZoneIcon|熔岩湖}}, {{ZoneIcon|被遗忘的冻土}}或{{ZoneIcon|雾气湖}}）非常有用。
* {{ItemIcon|穿透药水 IV}} 可能是从药水中获得的最高原始DPS，并且一旦解锁，可以在轻松获取{{ItemIcon|幽灵粘液}}后，立即用作所有扩展战斗的默认战斗药水。
* {{ItemIcon|暴击药水 IV}} 在大多数情况下与穿透药水相当，但需要一种更容易获得的材料，而无需进行困难的战斗。
* {{ItemIcon|自适应防御药水 IV|notext=true}}{{ItemIcon|自适应精准药水 IV|notext=true}} 自适应药水具有最初在{{MonsterIcon|飞马}}上找到的机制，通常不应使用这些药水，因为它们的效果实际上会在玩家的技能[Hitpoints](#)低于其最大值的57％以下时“降低”玩家的属性，并且效果相对于众多可用选项而言效果不佳。

===召唤===

一些特定的协同效应值得注意：
* {{ItemIcon|牛头怪|notext=true}}{{ItemIcon|半人马|notext=true}}{{ItemIcon|女巫|notext=true}}和{{ItemIcon|幽灵}} - 针对具有较高减伤的敌人，提供大量额外伤害。
* {{ItemIcon|牛头怪|notext=true}}{{ItemIcon|半人马|notext=true}}{{ItemIcon|女巫|notext=true}}和{{ItemIcon|狐狸}} - 针对减伤较低的敌人，提供大量额外伤害。
* {{ItemIcon|牛头怪|notext=true}}{{ItemIcon|半人马|notext=true}}{{ItemIcon|女巫|notext=true}}和{{ItemIcon|塞壬}} - 额外伤害，并附带CC的好处，对于多次攻击特殊攻击最有用 - 睡着的怪物无法造成伤害！
* {{ItemIcon|邪教徒}}和{{ItemIcon|幽灵}} - 通常会提供疯狂的原始准确性和命中几率，这是少数几个地方之一，其中仅仅获得更多准确性实际上是一个有形的好处，而不是微小的力量变化之一。
* {{ItemIcon|狐狸}}和{{ItemIcon|龙}} - 针对生命值较高的敌人，特别适用于多次攻击特殊攻击。
* {{ItemIcon|蜘蛛}}和{{ItemIcon|龙}} - 特别适用于配合{{ItemIcon|阻碍药水 IV}}和{{ItemIcon|皇家毒液长矛}}、{{SpellIcon|毒性荚囊}}或{{SpellIcon|自然之花}}使用。

===可消耗物品槽===

* {{ItemIcon|探险者地图}}和{{ItemIcon|探险者地图 II}} 提供了一组非常强大的[杀戮](#)增益效果。
* {{ItemIcon|磨刀石}} 可让玩家造成更多的[近战](#)伤害。
* {{ItemIcon|巫师卷轴}}和{{ItemIcon|巫师卷轴 II}} 可以保留符文。
* {{ItemIcon|杀戮之火炬}}和{{ItemIcon|神秘灯笼}} 用于解锁{{ZoneIcon|受诅咒之城}}和{{ZoneIcon|午夜山谷}} [杀戮](#)区域。玩家可以通过装备{{ItemIcon|卓越杀戮技能披风}}来避免使用这些可消耗物品 - {{ItemIcon|杀戮技能披风}} 不适用于这些物品。

==使者的王座战斗==

===早期游戏中的使者的王座：100级至110级===

在击败了{{MonsterIcon|恐惧之器贝恩}}（Bane, Instrument of Fear）于{{ZoneIcon|迫在眉睫的黑暗事件}}中后，玩家将解锁穿戴所有100级及以上的武器和盔甲的能力。立即的目标将是装备全新的装备，并开始通过击败{{ZoneIcon|丛林迷宫}}中的怪物以解锁所需的线索，从而解锁{{ZoneIcon|古老圣殿}}。

====主要目标====
* 从非战斗技能中获得新的武器和盔甲。所有等级达到100以上的盔甲都是直接升级版本，具有更好的攻击统计数据和伤害减免，分别是 {{ItemIcon|Aeris God Helmet|Aeris}}, {{ItemIcon|Glacia God Helmet|Glacia}}, 以及 {{ItemIcon|Terran God Helmet|Terran}} 神圣盔甲。不幸的是，{{ItemIcon|Ragnar God Helmet|Ragnar}} 神圣盔甲的额外准确性和伤害会丧失，但大量的额外伤害减免绝对是值得的：
  ** [Smithing](#) 用于制作 {{ItemIcon|Corundum Helmet|Corundum}}, {{ItemIcon|Augite Helmet|Augite}}, {{ItemIcon|Meteorite Helmet|Meteorite}}, 以及 {{ItemIcon|Divine Helmet|Divinite}} 盔甲和武器。所有盔甲除了 {{ItemIcon|Meteorite Helmet|Meteorite}} 必须使用 {{ItemIcon|Iridium Bar|Iridium}} 和 {{ItemIcon|Palladium Bar|Palladium}} 条来升级到其最大值。
  ** [Crafting](#) 用于制作 {{ItemIcon|Elderwood Body|Elderwood}}, {{ItemIcon|Revenant Body|Revenant}}, 以及 {{ItemIcon|Carrion Body|Carrion}} 盔甲和武器。所有盔甲必须使用制作原始物品时使用的相同原木升级到其最大值。{{ItemIcon|Carrion Body|Carrion}} 盔甲需要从 {{ZoneIcon|Ancient Sanctuary}} 获取的 {{ItemIcon|Carrion Bark}}。
  ** [Runecrafting](#) 用于制作 {{ItemIcon|Poison Mythical Wizard Hat|Poison}}, {{ItemIcon|Lightning Mythical Wizard Hat|Lightning}}, 以及 {{ItemIcon|Infernal Mythical Wizard Hat|Infernal}} 大师、传奇和神话等级的巫师法袍。法袍不需要升级以获取新的统计数据。还有新的 {{ItemIcon|Archaic Rune|Runes}} 和 {{ItemIcon|Archaic Wand|Weapons}}，用于促进新的古老魔法的使用，第一个古老魔法在打败 {{ZoneIcon|Ancient Sanctuary}} 后解锁。
* {{ZoneIcon|Jungle Labyrinth}} 的怪物掉落购买 {{UpgradeIcon|Labyrinth Solution}} 所需的物品，以及最早的装备升级选项。
* {{ZoneIcon|Ancient Sanctuary}} 掉落一些物品并解锁扩展的其余部分。
* {{ZoneIcon|Lava Lake}} 和 {{ZoneIcon|Underground Lava Lake}} 掉落 {{ItemIcon|Burning Protection Material}} 和 {{ItemIcon|Ward of Flame Material}}，以及 {{ItemIcon|Blazing Helmet}}。
* {{ZoneIcon|Forsaken Tundra}} 和 {{ZoneIcon|Lightning Region}} 掉落 {{ItemIcon|Freezing Touch Material}} 以及制作 {{ItemIcon|Frostspark Boots}}, {{ItemIcon|FrostSpark 1H Sword}}, 以及 {{ItemIcon|FrostSpark Amulet}} 所需的各种零件。

====近战====
通常情况下，近战训练应该在{{MonsterIcon|Alraune}}上进行，或者在{{ZoneIcon|Ancient Sanctuary}}中进行，这取决于玩家的长期目标。

====远程====
前几个杀手区域有很多潜在的[Magic](#)怪物可以杀死，以获取[Ranged](#)经验。{{MonsterIcon|Spectral Ice Wolf}}更可取，因为要获得{{ItemIcon|FrostSpark 1H Sword}}所需的{{ItemIcon|Spectral Ice Sword Shard|Spectral Ice Sword Shards}}平均需要击败约10,000只{{MonsterIcon|Spectral Ice Wolf|Wolves}}，而没有物品翻倍。

====魔法====
前几个杀手区域有很多潜在的[Melee](#)怪物可以杀死，以获取[Magic](#)经验。不幸的是，它们都不掉落稀有物品，或者需要很长时间才能获得，所以选择取决于哪种方式最方便，或者哪种方式提供最佳的经验值。

===中期使者的王座：111-120级===
在这里，玩家的目标是收集尽可能多的特殊装备，以完全装备他们的角色，为战斗的最后阶段做准备。这将包括所有非最终Boss的地牢，以及除了最后两个杀手区域以外的所有内容。

====主要目标====
* {{ZoneIcon|Foggy Lake}}的怪物掉落{{ItemIcon|Allure Amulet}}、{{ItemIcon|Bleed Burner Cape}}，以及制作{{ItemIcon|Leviathan Shield}}的零件。
* {{MonsterIcon|Goliath Werewolf}}掉落{{ItemIcon|Gauntlets of Rage}}。
* {{ZoneIcon|Lair of the Spider Queen}}掉落强大的{{ItemIcon|Royal Toxins Spear}}。
* {{ZoneIcon|Cursed Forest}}掉落强大的{{ItemIcon|Mask of Madness}}、{{ItemIcon|Mask of Torment}}和{{ItemIcon|Mask of Despair}}，专为[Attack|Melee](#)、[Ranged](#)和[Magic](#)设计。这些面具可以在游戏的其余部分佩戴。
* {{ZoneIcon|Necromancers Palace}}掉落的物品并不特别强大，可以出于完成任务的目的而避免。
* {{ZoneIcon|Midnight Valley}}的怪物掉落{{ItemIcon|Ethereal Greataxe}}、{{ItemIcon|Ethereal Longbow}}和{{ItemIcon|Ethereal Staff}}，可以帮助突破其他中后期怪物的坚固伤害减免。每个怪物还有{{ItemIcon|Ectoplasm}}作为它们的骨头掉落，可以用来制作{{ItemIcon|Spectre}}召唤符。

====近战====
通常来说，{{MonsterIcon|Cockatrice}}将是最佳的经验来源，但在专注追求经验之前，确保您获得了所需的物品非常重要。

====远程====
通常来说，{{MonsterIcon|Siren}}将是最佳的经验来源，但在专注追求经验之前，确保您获得了所需的物品非常重要。

====魔法====
通常来说，{{MonsterIcon|Gret-Yun}}将是最佳的经验来源，但在专注追求经验之前，确保您获得了所需的物品非常重要。

===晚期游戏 - 使者的王座：118-120级===
在这里，我们将击败游戏中最终、最强大的怪物，然后杀死{{MonsterIcon|The Herald}}，结束他的统治。
====主要目标====
* {{MonsterIcon|Torvair}}，{{MonsterIcon|Arctair}}和{{MonsterIcon|Harkair}} 掉落游戏中最强大的武器的碎片 - {{ItemIcon|Agile Wings Rapier}}，{{ItemIcon|Feather Storm Crossbow}}和{{ItemIcon|Slicing Maelstrom Wand}}。这些怪物的指南可以在[[Golden Cloud Mountains/Guide]]找到。
* {{MonsterIcon|Vorloran Protector}}，{{MonsterIcon|Vorloran Watcher}}和{{MonsterIcon|Vorloran Devastator}} 掉落游戏中最高伤害减免的盔甲 - {{ItemIcon|Vorloran Protector Helmet|Vorloran Protector Armor}}，{{ItemIcon|Vorloran Watcher Helmet|Vorloran Watcher Armor}}和{{ItemIcon|Vorloran Devastator Helmet|Vorloran Devastator Armor}}。这些怪物的指南可以在[[Millennium Gate/Guide]]找到。
* {{ZoneIcon|Throne of the Herald}} 包含{{MonsterIcon|The Herald}}。这个地下城的指南可以在[[Throne of the Herald (Dungeon)/Guide]]找到。
